...
- **Статусы процесса.** Добавлен статус «На доработке» с возможностью возврата заявки инициатору и переработана панель смены статусов — теперь это диалог с описанием каждой стадии.
- **Обновлённый PDF-шаблон.** Письмо оформляется в стиле служебной заявки с верхним адресным блоком; параметры (получатель, должность, текст вступления и подпись) управляются через новые переменные `.env`.
- **Обновлённый UI.** Кабинет пользователя и панель администратора получили просторные карточные макеты, облегчающие работу на ноутбуках и широких экранах.
- **Расширенные справочники.** В `schema.sql` предзагружены популярные единицы измерения и материалы, а на форме заявки единица подставляется автоматически при выборе материала или категории.
- **PDF-кеш с автообновлением.** При изменении заявки кешированный PDF перегенерируется автоматически при следующей выдаче.
