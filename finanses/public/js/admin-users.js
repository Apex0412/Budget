...
async function loadUsers() {
    const response = await fetch('/finanses/public/api/users.php');
...
document.getElementById('saveUser')?.addEventListener('click', async () => {
    ...
    let data;
    try {
        data = await window.App.request('/finanses/public/api/users.php', {
            method: 'POST',
...
usersTable?.addEventListener('click', async (event) => {
    ...
    if (button.dataset.action === 'reset') {
        try {
            const data = await window.App.request(`/finanses/public/api/users.php?id=${id}&action=reset-password`, { method: 'PATCH' });
    ...
    if (button.dataset.action === 'delete') {
        ...
        try {
            await window.App.request(`/finanses/public/api/users.php?id=${id}`, { method: 'DELETE' });
